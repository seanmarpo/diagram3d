...
function tokeniseLine(line) {
  ...
  while (i < n) {
    ...
    // consume id (word chars, hyphens allowed)
    ...
    if (j < n && '([{>'.includes(line[j])) {
      ...
      while (j < n) {
        ...
        else if (line[j] === close) { depth--; if (depth === 0) { j++; break; } }
        j++;
      }
      // handle ((...)) and [(...)]
      if (j < n && line[j] === open) {
...
function parseNodeToken(token) {
  ...
  return { id, label: suffix.replace(/^[\[({>]|[\])}]$/g, '').trim() || id, shape: 'rect' };
}
...
export function parseMermaid(src) {
  ...
  for (const raw of lines) {
    ...
    // ── Subgraph ─────────────────────────────────────────────────
    if (/^subgraph\b/i.test(line)) { inSubgraph = true;  continue; }
    if (/^end\b/i.test(line))      { inSubgraph = false; continue; }
    ...
    for (const t of firstIds) {
      ...
      if (p) { ensureNode(p.id, p.label, p.shape); srcNodes.push(p.id); }
    }
    i++;
    ...
    while (i < tokens.length - 1) {
      ...
      for (const t of tgtIds) {
        const p = parseNodeToken(t);
        if (p) { ensureNode(p.id, p.label, p.shape); dstNodes.push(p.id); }
      }
